...
export default function MemberActions({ memberId, status, isAdmin }: { memberId: string; status: string; isAdmin: boolean }) {
  const router = useRouter()
  const [loading, setLoading] = useState<string | null>(null)

  async function act(action: 'approve' | 'disable' | 'enable' | 'toggle_admin') {
    setLoading(action)
      ...
      {status === 'approved' && (
        <>
          <button onClick={() => act('toggle_admin')} disabled={!!loading}
            ...
            {loading === 'toggle_admin' ? '…' : isAdmin ? 'Admin ✓' : 'Make admin'}
          </button>
          <button onClick={() => act('disable')} disabled={!!loading}
